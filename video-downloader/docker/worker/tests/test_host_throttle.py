...
from host_throttle import HostThrottle, _LUA_TRY_ACQUIRE
...
def test_acquire_succeeds_below_cap():
    redis = _FakeRedis()
    throttle = HostThrottle(redis, cap=3)
...
def test_acquire_blocks_at_cap_then_proceeds_after_release():
    ...
    redis = _FakeRedis()
    throttle = HostThrottle(redis, cap=2, max_wait=5.0)
...
def test_acquire_times_out_returns_false():
    ...
    redis = _FakeRedis()
    throttle = HostThrottle(redis, cap=1, max_wait=0.3)
    assert throttle.acquire("https://host.test/x") is True
...
def test_release_decrements_and_clamps_negative():
    ...
    redis = _FakeRedis()
    throttle = HostThrottle(redis, cap=2)
...
def test_acquire_with_redis_error_returns_false_does_not_block():
    ...
    throttle = HostThrottle(_BrokenRedis(), cap=4)
...
def test_separate_hosts_have_separate_caps():
    ...
    redis = _FakeRedis()
    throttle = HostThrottle(redis, cap=1)
...
def test_concurrent_acquires_respect_cap():
    ...
    redis = _FakeRedis()
    throttle = HostThrottle(redis, cap=10, max_wait=0.05)
